...
class LinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> 'LinearRegression':
        ...
        print('R^2 = ', self.residual_sum_squares)
        self.fitted = True
        return self
...
if __name__ == '__main__':
    ...
    x = hydropower['BCR'].to_numpy()
    y = hydropower['AnnualProduction'].to_numpy()
    fig, ax = plt.subplots(figsize = (5,6))
    plt.scatter(x, y, c = 'orange', edgecolors = 'blue', s = 200)
    fig.patch.set_alpha(0.0)
    plt.savefig('./Hydropower_Scatter.pdf', transparent = True)

    model = LinearRegression(learning_rate=0.01, max_epochs=10000) # Construcotr __init__
    model.fit(x, y) # Fitting the function

    w1, w0 = model.get_parameters()
    print(f"Trained model: y  = {w1:.4f} + {w0:.4f}x")
   
    #Prediction
    y_prediction = model.predict(np.array([250]))
    print(f"Predicted value for x = 250 is y = {y_prediction[0]:.4f}")
